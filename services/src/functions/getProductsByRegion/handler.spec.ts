...
describe('Get Products By Region Handler', () => {
  ...
  it('should fetch products filtered by region with an optimized query when querystring useOptimizedQuery is enabled', async () => {
    const context = {} as Context;
    const callback = null as Callback;
    ...
    const event = createEvent('aws:apiGateway', {
      pathParameters: { id: region.id },
      queryStringParameters: { useOptimizedQuery: 'true' },
    });
